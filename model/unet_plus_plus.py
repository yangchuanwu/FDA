...
class NestNet(nn.Module):
    ...
    def forward(self, inputs, L=4):
        ...
        if L == 4:
            if self.deep_supervision:
                return torch.stack([x_01_output, x_02_output, x_03_output, x_04_output], 1),\
                       (nestnet_output_1 + nestnet_output_2 + nestnet_output_3 + nestnet_output_4) / 4
            else:
